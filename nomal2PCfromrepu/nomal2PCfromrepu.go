...
import (
	"code/config"
	"code/nomal2PCfromrepu/client"
	"code/nomal2PCfromrepu/shard1"
	"code/nomal2PCfromrepu/shard2"
	"code/nomal2PCfromrepu/shard3"
	"code/nomal2PCfromrepu/shard4"
	"code/nomal2PCfromrepu/shard5"
	"code/nomal2PCfromrepu/shard6"
	"code/nomal2PCfromrepu/shard7"
	"strconv"
	"sync"
	"time"
)
...
func main() {
	...
	go nomal2PCfromrepu.ListenForFeedback()

	nomal2PCfromrepu.ExitChan = make(chan bool) // Initialize the exit channel
	go func() {
		nomal2PCfromrepu.StartClient()
	}()

	// Block main goroutine to keep the process running until exitChan receives a signal
	<-nomal2PCfromrepu.ExitChan
}
