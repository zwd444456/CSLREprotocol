...
type Node struct {
	...
	ShardCertificates     map[int]map[int]bool
	ConfirmedTransactions map[int]bool
	consensusRequired     int // number of nodes required for consensus
	consensusVotes        map[int]map[int]bool
	leaderTimeouts        map[int]*time.Timer
	mu                    sync.Mutex
	wg                    *sync.WaitGroup
	leaderChangeActive    bool
}
...
func NewNode(id int, wg *sync.WaitGroup) *Node {
	return &Node{
		...
		NumCertificatesNeeded: config.ShardNumber - 1,
...
func (n *Node) CheckShard2Leader(transactionID int) {
	log.Printf("time=\"%s\" level=info msg=\"Leader Node %d checking Shard 2 leader for transaction %d.\"\n", time.Now().Format(time.RFC3339), n.ID, transactionID)
...
func (n *Node) ReceiveVote(w http.ResponseWriter, r *http.Request) {
	...
	time.Sleep(time.Millisecond * time.Duration(config.Delay) / 10)
