...
func (n *Node) SendPrepare(transactionID int) {
	...
	for i := config.Shard4Leader; i <= config.Shard4Leader+config.Node-1; i++ { // Send to other nodes
		go func(i int) {
...
func (n *Node) SendCommit(transactionID int) {
	...
	for i := config.Shard4Leader; i <= config.Shard4Leader+config.Node-1; i++ {
		go func(i int) {
...
func (n *Node) ReceiveCommit(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		_, err := http.Get(fmt.Sprintf("http://localhost:%d/commit_ack?transactionID=%d&nodeID=%d&payload=%s", config.Shard4Leader, transactionID, n.ID, config.GlobalPayload))
		if err != nil {
