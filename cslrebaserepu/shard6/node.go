...
type Node struct {
	...
	mu                sync.Mutex
	wg                *sync.WaitGroup
	receivedResults   map[int]map[int]int
	receivedAcks      map[int]map[int]bool
	consensusRequired int
	prepareCerts      map[int]int
	voteCerts         map[int]int
	commitCerts       map[int]int
...
func NewNode(id int, wg *sync.WaitGroup) *Node {
	return &Node{
		...
		consensusRequired: config.Node/2 + 1,
		prepareCerts:      make(map[int]int),
		voteCerts:         make(map[int]int),
		commitCerts:       make(map[int]int),
...
func (n *Node) SendResultToLeader(transactionID, result int) {
	if n.IsLeader {
		...
		go func() {
			_, err := http.Get(fmt.Sprintf("http://localhost:%d/result?transactionID=%d&nodeID=%d&result=%d&payload=%s", config.Shard6Leader, transactionID, n.ID, result, config.GlobalPayload))
			if err != nil {
...
func (n *Node) SendPrepare(transactionID int) {
	...
	for i := config.Shard6Leader; i <= config.Shard6Leader+config.Node-1; i++ { // Send to other nodes
		go func(i int) {
...
func (n *Node) ReceivePrepare(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		_, err := http.Get(fmt.Sprintf("http://localhost:%d/prepare_ack?transactionID=%d&nodeID=%d&payload=%s", config.Shard6Leader, transactionID, n.ID, config.GlobalPayload))
		if err != nil {
...
func (n *Node) ReceiveVote(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		log.Printf("time=\"%s\" level=info msg=\"Node %d send vote ack certificate %d for transaction %d to leader %d \"\n", time.Now().Format(time.RFC3339), n.ID, certificate, transactionID, config.Shard6Leader)
		_, err := http.Get(fmt.Sprintf("http://localhost:%d/vote_ack?transactionID=%d&nodeID=%d&payload=%s", config.Shard6Leader, transactionID, n.ID, config.GlobalPayload))
		if err != nil {
...
func (n *Node) CheckVoteAcks(transactionID int) {
	if len(n.receivedAcks[transactionID]) >= n.consensusRequired {
		...
		n.SendCommit(transactionID)
	}
}

func (n *Node) SendCommit(transactionID int) {
	certificate := n.receivedResults[transactionID][n.ID]
	n.commitCerts[transactionID] = certificate
	log.Printf("time=\"%s\" level=info msg=\"Leader Node %d sending commit certificate %d for transaction %d\"\n", time.Now().Format(time.RFC3339), n.ID, certificate, transactionID)

	for i := config.Shard6Leader; i <= config.Shard6Leader+config.Node-1; i++ {
		go func(i int) {
			_, err := http.Get(fmt.Sprintf("http://localhost:%d/commit?transactionID=%d&certificate=%d&payload=%s", i, transactionID, certificate, config.GlobalPayload))
			if err != nil {
				log.Printf("time=\"%s\" level=error msg=\"Error sending commit certificate to Node %d: %v\"\n", time.Now().Format(time.RFC3339), i, err)
			}
		}(i)
	}
}

func (n *Node) ReceiveCommit(w http.ResponseWriter, r *http.Request) {
	transactionIDStr := r.URL.Query().Get("transactionID")
	certificateStr := r.URL.Query().Get("certificate")
	time.Sleep(time.Millisecond * time.Duration(config.Delay))
	transactionID, _ := strconv.Atoi(transactionIDStr)
	...
	if n.completed[transactionID] {
		log.Printf("time=\"%s\" level=info msg=\"Node %d ignored commit certificate %d for transaction %d because consensus is already reached\"\n", time.Now().Format(time.RFC3339), n.ID, certificate, transactionID)
		return
	}

	log.Printf("time=\"%s\" level=info msg=\"Node %d received commit certificate %d for transaction %d\"\n", time.Now().Format(time.RFC3339), n.ID, certificate, transactionID)

	go func() {
		_, err := http.Get(fmt.Sprintf("http://localhost:%d/commit_ack?transactionID=%d&nodeID=%d&payload=%s", config.Shard6Leader, transactionID, n.ID, config.GlobalPayload))
		if err != nil {
			log.Printf("time=\"%s\" level=error msg=\"Error sending commit ack to leader: %v\"\n", time.Now().Format(time.RFC3339), err)
		}
	}()
}

func (n *Node) ReceiveCommitAck(w http.ResponseWriter, r *http.Request) {
	transactionIDStr := r.URL.Query().Get("transactionID")
	nodeIDStr := r.URL.Query().Get("nodeID")
	...
	if n.completed[transactionID] {
		log.Printf("time=\"%s\" level=info msg=\"Leader Node %d ignored commit ack for transaction %d from Node %d because consensus is already reached\"\n", time.Now().Format(time.RFC3339), n.ID, transactionID, nodeID)
		return
	}
	...
	log.Printf("time=\"%s\" level=info msg=\"Leader Node %d received commit ack from Node %d for transaction %d\"\n", time.Now().Format(time.RFC3339), n.ID, nodeID, transactionID)

	n.CheckCommitAcks(transactionID)
}

func (n *Node) CheckCommitAcks(transactionID int) {
	if len(n.receivedAcks[transactionID]) >= n.consensusRequired {
		log.Printf("time=\"%s\" level=info msg=\"Leader Node %d reached commit consensus for transaction %d\"\n", time.Now().Format(time.RFC3339), n.ID, transactionID)
...
func (n *Node) SendCertificateToShard3(transactionID int) {
	...
	commitCert := n.commitCerts[transactionID]
	log.Printf("time=\"%s\" level=info msg=\"Leader Node %d sending prepare certificate %d, vote certificate %d, and commit certificate %d for transaction %d to Shard 3\"\n", time.Now().Format(time.RFC3339), n.ID, prepareCert, voteCert, commitCert, transactionID)
	...
	if err != nil {
		...
		n.completed[transactionID] = true
	}
}
...
func (n *Node) Listen(port int) {
	...
	mux.HandleFunc("/commit", n.ReceiveCommit)
	mux.HandleFunc("/commit_ack", n.ReceiveCommitAck)
