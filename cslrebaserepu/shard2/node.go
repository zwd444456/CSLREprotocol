...
type Node struct {
	...
	leaderChangeCerts map[int]bool
}
...
func NewNode(id int, wg *sync.WaitGroup) *Node {
	return &Node{
		...
		consensusRequired: config.Node / 2,
...
func (n *Node) ReceiveCommit(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		_, err := http.Get(fmt.Sprintf("http://localhost:%d/commit_ack?transactionID=%d&nodeID=%d&payload=%s", leaderID+8000, transactionID, n.ID, config.GlobalPayload))
		if err != nil {
...
func (n *Node) SendCertificateToShard3(transactionID int) {
	...
	_, err := http.Get(fmt.Sprintf("http://localhost:%d/certificate?transactionID=%d&prepareCert=%d&voteCert=%d&commitCert=%d&shard=%d&payload=%s", config.Shard3Leader, transactionID, prepareCert, voteCert, commitCert, n.ID, config.GlobalPayload))
	if err != nil {
...
func (n *Node) Listen(port int) {
	...
	mux.HandleFunc("/view_change", n.ReceiveViewChange)
