...
import (
	CSVCbaserepu "code/CSVCbaserepu/client"
	"code/CSVCbaserepu/shard1"
	"code/CSVCbaserepu/shard2"
	"code/CSVCbaserepu/shard3"
	"code/CSVCbaserepu/shard4"
	"code/CSVCbaserepu/shard5"
	"code/CSVCbaserepu/shard6"
	"code/CSVCbaserepu/shard7"
	"code/config"
	"strconv"
	"sync"
	"time"
...
func main() {
	...
	go CSVCbaserepu.ListenForFeedback()
	CSVCbaserepu.ExitChan = make(chan bool) // Initialize the exit channel
	go func() {
		CSVCbaserepu.StartClient()
	}()

	// Block main goroutine to keep the process running until exitChan receives a signal
	<-CSVCbaserepu.ExitChan
}
