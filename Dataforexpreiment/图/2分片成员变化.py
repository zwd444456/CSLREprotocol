...
# 读取 Excel 文件中的数据
data = pd.read_excel("2分片成员变化的时间复杂度.xlsx")
...
plt.plot(x, data['协议23'], label='RBSMR-CSLRE', marker='^', color=colors['RBSMR-CSLRE'], markersize=10)  # 灰色
 # 灰色
plt.xticks([4,5, 6,7, 8])  # 只显示 4, 6, 8 这些坐标
# 设置 x 和 y 轴的标签
plt.xlabel('Shard Members',fontsize=25)  # 分片成员数量
plt.ylabel('Latency (ms)',fontsize=25)  # 延迟（毫秒）
