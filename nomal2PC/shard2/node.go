...
func (n *Node) SendResultToLeader(transactionID, result int) {
	if n.IsLeader {
		...
		go func() {
			_, err := http.Get(fmt.Sprintf("http://localhost:%d/result?transactionID=%d&nodeID=%d&result=%d&payload=%s", config.Shard2Leader, transactionID, n.ID, result, config.GlobalPayload))
			if err != nil {
...
func (n *Node) SendPrepare(transactionID int) {
	...
	for i := config.Shard2Leader; i <= config.Shard3Leader-1; i++ { // Send to other nodes
		go func(i int) {
...
func (n *Node) ReceivePrepare(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		_, err := http.Get(fmt.Sprintf("http://localhost:%d/prepare_ack?transactionID=%d&nodeID=%d&payload=%s", config.Shard2Leader, transactionID, n.ID, config.GlobalPayload))
		if err != nil {
...
func (n *Node) ReceiveVote(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		log.Printf("time=\"%s\" level=info msg=\"Node %d send vote ack certificate %d for transaction %d to leader %d \"\n", time.Now().Format(time.RFC3339), n.ID, certificate, transactionID, config.Shard2Leader)
		_, err := http.Get(fmt.Sprintf("http://localhost:%d/vote_ack?transactionID=%d&nodeID=%d&payload=%s", config.Shard2Leader, transactionID, n.ID, config.GlobalPayload))
		if err != nil {
