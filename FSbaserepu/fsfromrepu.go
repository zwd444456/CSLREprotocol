...
import (
	Fsbaserepu "code/FSbaserepu/client"
	"code/Fsbaserepu/shard1"
	"code/Fsbaserepu/shard2"
	"code/Fsbaserepu/shard3"
	"code/Fsbaserepu/shard4"
	"code/Fsbaserepu/shard5"
	"code/Fsbaserepu/shard6"
	"code/Fsbaserepu/shard7"
	"code/config"
	"strconv"
	"sync"
	"time"
...
func main() {
	...
	go Fsbaserepu.ListenForFeedback()
	Fsbaserepu.ExitChan = make(chan bool) // Initialize the exit channel
	go func() {
		Fsbaserepu.StartClient()
	}()
